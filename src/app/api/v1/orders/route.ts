...
function parsearDireccion(addressLine: string): {
  ...
  if (!addressLine)
    return { addressStreet: "", addressComuna: "", addressRegion: "" };
...
export async function POST(req: NextRequest) {
  ...
  if (!apiKey)
    return NextResponse.json(
      { ok: false, error: "API Key inválida o no enviada" },
      { status: 401 },
    );
  ...
  if (!customerName)
    return NextResponse.json(
      { ok: false, error: "customerName es requerido" },
      { status: 400 },
    );
  ...
  if (!resolvedStreet)
    return NextResponse.json(
      { ok: false, error: "addressStreet o address es requerido" },
      { status: 400 },
    );
  if (!resolvedComuna)
    return NextResponse.json(
      { ok: false, error: "addressComuna requerido" },
      { status: 400 },
    );
  ...
  if (!resolvedStoreId) {
    ...
    if (!firstStore)
      return NextResponse.json(
        { ok: false, error: "No hay tiendas disponibles" },
        { status: 400 },
      );
  ...
  try {
    ...
    // (antes del despacho automático, para que el webhook a Senby lleve su ID)
    ...
    let statusFinal: string = order.status;
    if (TIENDAS_AUTO_NOW.has(resolvedStoreId)) {
      try {
        const auto = await despachoAutomaticoNow([order.id], "auto-now");
        const actual = await prisma.order.findUnique({
          where: { id: order.id },
          select: { status: true },
        });
        statusFinal = actual?.status ?? statusFinal;
        if (auto.errores.length > 0) {
          console.warn(
            "[API v1] Now rechazó el pedido, queda RECEIVED:",
            order.orderNumber,
            auto.errores[0].error,
          );
        }
      } catch (autoErr) {
        // El pedido ya quedó creado: no fallar la respuesta a Senby por esto
        console.error(
          "[API v1] Error en despacho automático:",
          order.orderNumber,
          autoErr,
        );
      }
