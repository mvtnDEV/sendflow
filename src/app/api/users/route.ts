...
export async function POST(req: NextRequest) {
  ...
  const validRoles = ['SUPER_ADMIN','STORE_ADMIN','DRIVER']
  if (!validRoles.includes(body.role)) {
    return NextResponse.json({ ok:false, error:'Rol inválido' }, { status:400 })
  }

  // Validar contraseña / PIN según rol
  if (body.role === 'DRIVER') {
  ...
  const passwordHash = body.role === 'DRIVER'
    ? await hashPassword(body.pin)    // DRIVER: password = PIN
    : await hashPassword(body.password)

  const pinHash = body.role === 'DRIVER'
  ...
  await audit({
    userId:   me.id,
    action:   'CREATE_ORDER', // reusar — en producción agregar CREATE_USER
