...
export async function GET(req: NextRequest) {
  ...
  const pendientes = await prisma.order.findMany({
    ...
    select: {
      ...
      orderNumber: true,
      storeId: true,
      pendingNowEvidence: true,
    },
  });

  console.log(
    `[Cron ML] Revisando ${pendientes.length} pedidos pendientes de confirmación ML`,
  );
  ...
  for (const order of pendientes) {
    ...
    if (mlCheck.isDelivered) {
      ...
      console.log(
        "[Cron ML] Confirmado y cerrado como DELIVERED:",
        order.orderNumber,
      );
      results.push({
      ...
      if (esNoEntregado) {
        ...
        console.log(
          "[Cron ML] Cerrado como INCIDENT:",
          order.orderNumber,
          "|",
          mlCheck.shipmentStatus,
          mlCheck.shipmentSubstatus,
        );
        results.push({
          orderNumber: order.orderNumber,
          status: "closed_incident",
          mlStatus: mlCheck.shipmentStatus,
        });
      } else {
        await prisma.order.update({
          where: { id: order.id },
