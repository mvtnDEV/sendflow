...
const TYPE_STYLE: Record<AlertType, { bg: string; color: string }> = {
  ...
  STUCK_RECEIVED: { bg: "#DBEAFE", color: "#1E40AF" },
  DELIVERY_FAILED: { bg: "#FFE4E6", color: "#9F1239" },
};
const TYPE_HELP: Record<AlertType, string> = {
  FLEX_CANCELLED:
    "Mercado Libre Flex dejó de gestionar el envío. Revisar con el operador.",
  STUCK_IN_TRANSIT: "Lleva más de 24 horas en camino sin cerrarse.",
  STUCK_RECEIVED: "Lleva más de 12 horas recepcionado sin ponerse en camino.",
  DELIVERY_FAILED: "El pedido fue marcado como no entregado por el operador.",
};
...
export default function AlertasClient({ initial }: { initial: AlertRow[] }) {
  ...
  function cambiarTab(t: AlertStatus) {
    setTab(t);
    cargar(t, tipo);
  }
  function cambiarTipo(t: AlertType | "ALL") {
    setTipo(t);
    cargar(tab, t);
  }
  ...
  const contadores = {
    FLEX_CANCELLED: alerts.filter((a) => a.type === "FLEX_CANCELLED").length,
    STUCK_IN_TRANSIT: alerts.filter((a) => a.type === "STUCK_IN_TRANSIT")
      .length,
    STUCK_RECEIVED: alerts.filter((a) => a.type === "STUCK_RECEIVED").length,
    DELIVERY_FAILED: alerts.filter((a) => a.type === "DELIVERY_FAILED").length,
  };
    ...
    <div style={{ maxWidth: 1100 }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ fontSize: 13, color: "#6B7280", marginTop: 3 }}>
          Pedidos que necesitan una revisión manual. Se detectan solas cada 30
          minutos y se cierran solas cuando el pedido avanza.
        </p>
      </div>
      ...
      >
        {TIPOS.map((t) => (
          <div
            key={t}
            onClick={() => cambiarTipo(tipo === t ? "ALL" : t)}
            style={{
              padding: "10px 16px",
              background: TYPE_STYLE[t].bg,
              borderRadius: 10,
              cursor: "pointer",
              border:
                tipo === t
                  ? `2px solid ${TYPE_STYLE[t].color}`
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
        {(["ACTIVE", "RESOLVED"] as AlertStatus[]).map((t) => (
          ...
          >
            {t === "ACTIVE" ? `Activas (${alerts.length})` : "Resueltas"}
          </button>
        ))}
      </div>
          ...
          <div style={{ padding: 48, textAlign: "center" }}>
            ...
            <div style={{ fontSize: 13, color: "#6B7280", marginTop: 4 }}>
              {tab === "ACTIVE"
                ...
                : "Todavía no se ha resuelto ninguna alerta."}
            </div>
          </div>
        ) : (
          <table
              ...
              {visibles.map((a) => (
                <tr key={a.id} style={{ borderTop: "1px solid #F1F5F9" }}>
                  <td style={{ padding: "11px 14px", verticalAlign: "top" }}>
                    ...
                    <div
                      style={{ color: "#475569", marginTop: 5, maxWidth: 340 }}
                    >
                      {a.title}
                    </div>
                    {a.detail && (
