...
interface Props {
  searchParams: {
    ...
    dateFrom?: string
    dateTo?: string
    historial?: string   // "1" = ver todo el historial
    page?: string
  }
}
...
export default async function RecepcionesPage({ searchParams }: Props) {
  ...
  // Si no hay filtro de fecha explícito Y no pidió historial → solo hoy
  ...
  const [stats, result] = await Promise.all([
    getDashboardStats(storeId, todayOnly),
    listOrders({
      storeId,
    ...
    <div>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16, flexWrap:'wrap', gap:10 }}>
        ...
        <div style={{ display:'flex', gap:6 }}>
          <Link href="/recepciones"
            style={{ padding:'6px 14px', borderRadius:8, fontSize:12, fontWeight:500, textDecoration:'none',
      ...
      <div style={{ display:'flex', gap:8, marginBottom:12, flexWrap:'wrap', alignItems:'center' }}>
        <form method="GET" style={{ display:'flex', gap:8, flex:1, flexWrap:'wrap' }}>
          {/* Mantener parámetros activos */}
          {verTodo && <input type="hidden" name="historial" value="1"/>}

          {/* Buscador */}
          <div style={{ display:'flex', alignItems:'center', gap:8, background:'white', border:'1px solid #E2E8F0', borderRadius:8, padding:'7px 12px', flex:1, minWidth:220 }}>
          ...
          {/* Fechas (solo si está en historial) */}
          {verTodo && (<>
          ...
          {(searchParams.search || searchParams.status || searchParams.platform || searchParams.dateFrom) && (
          ...
          <div style={{ padding:'12px 16px', borderTop:'1px solid #E2E8F0', display:'flex', justifyContent:'space-between', alignItems:'center', fontSize:12, color:'#6B7280' }}>
            ...
            {result.totalPages > 1 && (
              <div style={{ display:'flex', gap:6, alignItems:'center' }}>
                ...
                {page > 1 && (
                  <Link href={`?page=${page-1}${verTodo?'&historial=1':''}${searchParams.status?`&status=${searchParams.status}`:''}${searchParams.search?`&search=${searchParams.search}`:''}`}
                    style={{ padding:'4px 10px', border:'1px solid #E2E8F0', borderRadius:6, textDecoration:'none', color:'#374151' }}>
                    ←
                  </Link>
                )}
