...
export default function RecepcionesClient({
  ...
  const [loadingFret, setLoadingFret] = useState(false);
  ...
  const flexEsperando = orders.filter(
    (o) => o.platform === "MERCADOLIBRE" && !o.mlShippedAt,
  );
  ...
  async function exportExcel() {
    ...
    try {
      ...
      const rows = data.data.map((o: any) => ({
        ...
        Entregado: o.deliveredAt
          ? new Date(o.deliveredAt).toLocaleString("es-CL")
          : "",
      }));
  ...
  async function exportExcelFlexEsperando() {
    if (flexEsperando.length === 0) {
      alert("No hay pedidos Flex esperando escaneo en esta vista.");
      return;
    }
  ...
  async function enviarAFretSeleccionados() {
    const ids =
      selected.size > 0
        ? Array.from(selected).filter((id) =>
            pendientes.some((o) => o.id === id),
        ...
        : pendientes.map((o) => o.id);
    if (ids.length === 0) {
      alert("Selecciona pedidos PENDING para enviar a Fret");
      return;
    }
    if (
      !confirm(
        `¿Enviar ${ids.length} pedido${ids.length !== 1 ? "s" : ""} a Fret?\n\nSe enviarán con punto_retiro: sendby y en_bodega: true`,
      )
    )
      return;
    setLoadingFret(true);
    setResultado(null);
    try {
      const res = await fetch("/api/orders/batch-send-fret", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ orderIds: ids }),
      });
      ...
      if (data.ok) {
        setResultado({
          ok: true,
          msg: `✅ ${data.enviados} pedido${data.enviados !== 1 ? "s" : ""} enviado${data.enviados !== 1 ? "s" : ""} a Fret`,
        });
        setSelected(new Set());
        setTimeout(() => router.refresh(), 1500);
      } else {
      ...
      setResultado({ ok: false, msg: "❌ Error al enviar a Fret" });
    } finally {
      setLoadingFret(false);
    }
  }
  ...
  async function ponerEnCaminoTodos() {
    ...
    if (
      !confirm(
        `¿Poner en camino ${recibidos.length} pedido${recibidos.length !== 1 ? "s" : ""} recepcionado${recibidos.length !== 1 ? "s" : ""}?\n\nEsto notificará a Senby que están en camino.`,
      )
    )
      return;
    ...
    try {
      ...
      if (data.ok) {
        setResultado({
          ok: true,
          msg: `✅ ${data.updated} pedido${data.updated !== 1 ? "s" : ""} en camino`,
        });
        setTimeout(() => router.refresh(), 1500);
      } else {
        setResultado({ ok: false, msg: `❌ Error: ${data.error}` });
      ...
      setResultado({ ok: false, msg: "❌ Error al poner en camino" });
    } finally {
      setLoadingEnCamino(false);
    }
  }

  async function imprimirEtiquetas() {
    const ids =
      selected.size > 0 ? Array.from(selected) : orders.map((o: any) => o.id);
    if (ids.length === 0) {
    ...
    <div>
      ...
      >
        {pendientes.length > 0 && (
          <>
            <button
              onClick={recepcionarTodos}
              disabled={loadingRecepcionar}
              style={{
                padding: "7px 14px",
                background: loadingRecepcionar ? "#93C5FD" : "#D97706",
                color: "white",
                border: "none",
                borderRadius: 8,
                fontSize: 13,
                fontWeight: 500,
                cursor: loadingRecepcionar ? "not-allowed" : "pointer",
              }}
            >
              {loadingRecepcionar
                ? `⏳ Recepcionando…`
            ...
            {isSuperAdmin && esSenby && (
              <button
                ...
                style={{
                  padding: "7px 14px",
                  background: loadingFret ? "#93C5FD" : "#2563EB",
                  color: "white",
                  border: "none",
                  borderRadius: 8,
                  fontSize: 13,
                  fontWeight: 500,
                  cursor: loadingFret ? "not-allowed" : "pointer",
                }}
              >
                {loadingFret
                  ? `⏳ Enviando a Fret…`
                  : `🚚 Enviar a Fret (${selected.size > 0 ? selected.size : pendientes.length})`}
              </button>
            )}
          </>
        )}
        {isSuperAdmin && esSenby && recibidos.length > 0 && (
          <button
            onClick={ponerEnCaminoTodos}
            disabled={loadingEnCamino}
            style={{
              padding: "7px 14px",
              background: loadingEnCamino ? "#86EFAC" : "#16A34A",
              color: "white",
              border: "none",
              borderRadius: 8,
              fontSize: 13,
              fontWeight: 500,
              cursor: loadingEnCamino ? "not-allowed" : "pointer",
            }}
          >
            {loadingEnCamino
              ? `⏳ Poniendo ${recibidos.length} en camino…`
              : `🚚 Poner en camino (${recibidos.length})`}
          </button>
        )}
        {orders.length > 0 && (
          <button
            ...
            style={{
              padding: "7px 14px",
              background: loadingEtiquetas ? "#93C5FD" : "#7C3AED",
              color: "white",
              border: "none",
              borderRadius: 8,
              fontSize: 13,
              fontWeight: 500,
              cursor: loadingEtiquetas ? "not-allowed" : "pointer",
            }}
          >
            {loadingEtiquetas
              ? "⏳ Generando..."
        ...
        <button
          ...
          style={{
            padding: "7px 14px",
            background: loadingExport ? "#86EFAC" : "#16A34A",
            color: "white",
            border: "none",
            borderRadius: 8,
            fontSize: 13,
        ...
        {isSuperAdmin && flexEsperando.length > 0 && (
          <button
            onClick={exportExcelFlexEsperando}
            disabled={loadingExportFlex}
            style={{
              padding: "7px 14px",
              background: loadingExportFlex ? "#FDE68A" : "#D97706",
              color: "white",
              border: "none",
              borderRadius: 8,
              fontSize: 13,
              fontWeight: 500,
              cursor: loadingExportFlex ? "not-allowed" : "pointer",
            }}
          >
            {loadingExportFlex
              ? "⏳..."
              : `⬇ Flex esperando (${flexEsperando.length})`}
          </button>
        )}
        {selected.size > 0 && (
          <button
      ...
      {resultado && (
        <div
          style={{
            ...
            borderRadius: 8,
            marginBottom: 8,
            background: resultado.ok ? "#F0FDF4" : "#FFF1F2",
            color: resultado.ok ? "#166534" : "#9F1239",
            border: `1px solid ${resultado.ok ? "#BBF7D0" : "#FECDD3"}`,
            fontWeight: 500,
          }}
        >
          {resultado.msg}
      ...
      {orders.length === 0 ? (
        ...
        >
          ...
          <div style={{ fontSize: 13, marginBottom: 20 }}>
            {todayOnly
              ? "Los pedidos que lleguen hoy aparecerán aquí automáticamente"
        ...
        <>
            ...
            >
              <thead>
                <tr style={{ background: "#F8FAFC" }}>
                  ...
                  {isStoreAdmin &&
                    userStoreId === "cmouw44ej0004thpecq6bct35" && (
                      <th
                        className="col-hide-mobile"
                        style={{
                          padding: "10px 12px",
              ...
              <tbody>
                {orders.map((order) => {
                  ...
                  const sourceLabel = formatSourceId(
                    order.sourceId,
                    order.platform,
                  );
                      ...
                      {isSuperAdmin && (
                        ...
                        >
                          {!esFlex ? (
                            ...
                            <span
                              title={`Escaneado por Flex: ${new Date(order.mlShippedAt).toLocaleString("es-CL", { timeZone: TZ })}`}
                              style={{
                                fontSize: 11,
                                padding: "2px 8px",
                                borderRadius: 20,
                            ...
                            <span
                              title="Aún no escaneado por el repartidor de Flex"
                              style={{
                                fontSize: 11,
                                padding: "2px 8px",
                                borderRadius: 20,
          ...
          >
            <span>
              ...
              {selected.size > 0 && (
                <span
                  style={{ marginLeft: 8, color: "#7C3AED", fontWeight: 500 }}
                >
