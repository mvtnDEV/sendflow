...
const COMUNAS_URBANAS = [
  'Santiago', 'Providencia', 'Las Condes', 'Vitacura', 'Ñuñoa', 'Macul',
  'San Miguel', 'La Florida', 'Puente Alto', 'Maipú', 'Estación Central',
  'Cerrillos', 'Cerro Navia', 'Conchalí', 'El Bosque', 'Huechuraba',
  'Independencia', 'La Cisterna', 'La Granja', 'La Pintana', 'La Reina',
  'Las Condes', 'Lo Barnechea', 'Lo Espejo', 'Lo Prado', 'Peñalolén',
  'Pedro Aguirre Cerda', 'Pudahuel', 'Quilicura', 'Quinta Normal',
  'Recoleta', 'Renca', 'San Joaquín', 'San Ramón', 'Colina', 'Lampa',
  'Til Til', 'Pirque', 'San José de Maipo', 'Buin', 'Calera de Tango',
  'Paine', 'San Bernardo', 'Talagante', 'El Monte', 'Isla de Maipo',
  'Padre Hurtado', 'Peñaflor',
]

const COMUNAS_EXTRA_URBANAS = [
  'Melipilla', 'Curacaví', 'María Pinto', 'San Pedro', 'Alhué',
  'Cartagena', 'El Quisco', 'El Tabo', 'San Antonio', 'Santo Domingo',
  'Algarrobo', 'Casablanca',
]

function getTipoTarifa(comuna: string): 'urbana' | 'extraUrbana' | 'rural' {
  const c = comuna.toLowerCase().trim()
  if (COMUNAS_URBANAS.some(u => u.toLowerCase() === c)) return 'urbana'
  if (COMUNAS_EXTRA_URBANAS.some(u => u.toLowerCase() === c)) return 'extraUrbana'
  return 'rural'
}
...
export default function FacturacionPage() {
  ...
  function calcFactura(sf: StoreFactura) {
    ...
    orders.forEach(o => {
      ...
      if (esRetiro) {
        const tarifa = store.tarifaRetiro ?? 0
        totalRetiro += tarifa
        countRetiro++
      } else {
        if (tipo === 'urbana') {
          const tarifa = store.tarifaUrbana ?? 0
          totalUrbana += tarifa
          countUrbana++
        } else if (tipo === 'extraUrbana') {
          const tarifa = store.tarifaExtraUrbana ?? 0
          totalExtraUrbana += tarifa
          countExtraUrbana++
        } else {
          const tarifa = store.tarifaRural ?? 0
          totalRural += tarifa
          countRural++
        }
      }
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:20, flexWrap:'wrap', gap:10 }}>
        <div>
          ...
          <p style={{ fontSize:13, color:'#6B7280', marginTop:3 }}>
            Pedidos entregados × tarifa por tipo de zona
          </p>
        </div>
      ...
      {/* ── Resumen total ── */}
      {data.length > 0 && (
        <div style={{ background:'#0B1628', borderRadius:12, padding:'16px 20px', marginBottom:20, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div>
