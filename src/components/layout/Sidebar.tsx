...
const ICONS: Record<string, React.ReactNode> = {
  ...
  chart:  <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M15 12H1v1h14v-1zM4 7L1 11h3V7zm3 2L4 7v4h3V9zm3-5L7 9v2h3V4zm3 3l-3-3v7h3V7z"/></svg>,
...
export default function Sidebar({ user, open, onClose }: {
  ...
  const sidebarContent = (
    <aside style={{
      width: 220, minWidth: 220, background: '#0B1628',
              ...
              .map(item => (
                <Link key={item.href} href={item.href} style={{
                  ...
                  {item.href === '/pedidos/carga-masiva' && (
                    <span style={{ marginLeft: 'auto', fontSize: 9, background: 'rgba(245,197,24,.2)', color: '#F5C518', padding: '1px 6px', borderRadius: 10, fontWeight: 500 }}>EXCEL</span>
                  )}
                  {item.href === '/integraciones' && (
                    <span style={{ marginLeft: 'auto', fontSize: 9, background: 'rgba(56,189,248,.15)', color: '#38BDF8', padding: '1px 6px', borderRadius: 10, fontWeight: 500 }}>API</span>
                  )}
                </Link>
              ))}
          </div>
      ...
      <div style={{ padding: 14, borderTop: '1px solid rgba(255,255,255,.07)', display: 'flex', alignItems: 'center', gap: 9 }}>
        ...
        <div style={{ flex: 1, overflow: 'hidden' }}>
          ...
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,.35)' }}>
            {user.role === 'SUPER_ADMIN' ? 'Super Admin' : user.role === 'VIEWER' ? 'Visualizador' : 'Tienda'}
          </div>
        </div>
